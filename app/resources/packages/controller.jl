...
function search()
  ...
  packages =  ! isempty(search_results) ?
              SearchLight.find(Package, SQLQuery(where = SQLWhere(:id, SQLInput(join( map(x -> string(x), search_results_df[:package_id]), ","), raw = true), "AND", "IN" ))) :
              []
...
using App, App.Packages
@dependencies

function index()
  packages_count = SearchLight.count(Package)
  Genie.config.model_relations_eagerness = RELATION_EAGERNESS_EAGER
